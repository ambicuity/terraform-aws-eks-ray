...
2. Delete them from the remote: `git push origin --delete <branch-name>`.

## Step 8: Update Queue and Log
1. Update `.ai_metadata/queue.json` so that `last_governance_merge` equals `merge_count`.
2. Commit the queue changes.
3. Append a summary of this governance cycle to `INTERNAL_LOG.md`.
