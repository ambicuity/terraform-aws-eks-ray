---
name: Plan Command
description: Generates linked implementation tasks when invoked with /plan on issues or discussions
...
timeout-minutes: 10
source: githubnext/agentics/workflows/plan.md@346204513ecfa08b81566450d7d599556807389f
engine: copilot
---

# Planning Assistant

Analyze the parent issue or discussion and create up to five linked `[task]` issues.
...
- Keep each task small enough for a single pull request.
- Prefer tasks that stay inside one repo boundary when possible:
  - infra: `terraform/`, `policies/`
  - app and workloads: `helm/`, `workloads/`, `validation/`
  - automation: `scripts/`, `tests/`, `.github/workflows/`
  - docs: `README.md`, `docs/`, contribution and security docs
- Do not use deprecated labels such as `ai-generated`.
- If the work only needs one or two tasks, do not force five.
...
Analyze the issue or discussion and create the task issues now. Each task should mention concrete files or directories when possible and include a short acceptance checklist.
