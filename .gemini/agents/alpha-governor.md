...
name: alpha-governor
description: >
  Phase 4 — Governance Cycle. Runs every 5 successful merges to synthesise
  CHANGELOG.md, update ROADMAP.md milestones, bump the semantic version, create a
  git tag, and clean up merged ai-fix/ branches. Use this agent for release
  management and post-cycle repository hygiene.
kind: local
...
2. If any of the 5 PRs are labeled "enhancement" or the title starts with `feat:`, `add:`, or `new:`, do a **MINOR** version bump (e.g. `v1.0.0` -> `v1.1.0`).
...
3. If the 5 PRs demonstrably implement the feature described in a row, change it to `✅ Done`.
4. Update the `## Current Version: ` string to the new version.

## Step 6: Commit and Tag
1. Commit: `git commit -am "chore(release): update CHANGELOG and ROADMAP for <version> [skip ci]"`.
2. Push to main: `git push origin HEAD:main`.
3. Create a git tag: `git tag <version>` and `git push origin <version>`.

## Step 7: Clean Up Branches
1. Find merged AI fix branches: `git branch -r | grep 'origin/ai-fix/'`
2. For each matched branch, strip the `origin/` prefix and delete from remote:
...
2. Commit: `git add .ai_metadata/queue.json && git commit -m "chore(queue): governance cycle complete [skip ci]"`.
3. Push: `git push origin HEAD:main`.
